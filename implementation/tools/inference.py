...
def compute_map(det_boxes, gt_boxes, iou_threshold=0.5, method='area', difficult=None):
    '''
    det_boxes = [ { 'person' : [[x1, y1, x2, y2, score], ...], 'car' : [[x1, y1, x2, y2, score], ...] }]
    gt_boxes = [ { 'person' : [[x1, y1, x2, y2], ...], 'car' : [[x1, y1, x2, y2], ...] }, ]
    '''
    ...
    for idx, label in enumerate(gt_labels):
        ...
        # cls_dets = [(0, [x1_0, y1_0, x2_0, y2_0, score_0])]
        ...
        for det_idx, (im_idx, det_pred) in enumerate(cls_dets):
            ...
            for gt_box_idx, gt_box in enumerate(im_gts):
                gt_box_iou = iou(det_pred[:-1], gt_box)
...
def convert_yolo_pred_x1y1x2y2(yolo_pred, S, B, C, use_sigmoid=False):
    """converts yolo predictions to x1y1x2y2 format """
